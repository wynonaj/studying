...
function numberTeachingInfo(x){
 const id=String(x.id||'').replace(/^lab-/,'');
 if(/^labels(?:10|200)$/.test(id))return {id:'notes-number-labels',title:'Bits, exponents & unique labels',rule:'A bit has two possible values: 0 or 1. Each extra bit doubles the number of possible labels. With n bits, there are 2ⁿ combinations. The small raised n is the exponent: it counts how many factors of 2 you multiply.',example:'With 3 bits: 2³ = 2 × 2 × 2 = 8 labels. To label 9 objects, 3 bits are too few; 4 bits give 16 labels. Do not subtract 2 for ordinary label counting.'};
 if(id==='framebytes')return {id:'notes-3-3',title:'Counting Ethernet frame bytes',rule:'Frame size = header + payload/padding + FCS. An ordinary untagged Ethernet header is 14 bytes and FCS is 4 bytes.',example:'A 100-byte payload makes a 14 + 100 + 4 = 118-byte frame. The preamble/SFD is outside this frame-size count.'};
 if(id.startsWith('ipv6'))return {id:'notes-4-3',title:'IPv6 groups & zero compression',rule:'IPv6 has eight groups of 16 bits. Each full group has four hex digits. :: replaces one consecutive stretch of all-zero groups.',example:'If five groups are written, :: fills in 8 − 5 = 3 zero groups. Leading zeros inside a group may also be omitted.'};
 if(id.startsWith('mac'))return {id:'notes-3-4',title:'Reading the two MAC address portions',rule:'A traditional universally assigned MAC address has six byte pairs. The first three pairs are the 24-bit organization prefix; the last three distinguish interfaces within that prefix.',example:'Count pairs, not individual hex digits: 3 pairs × 8 bits = 24 bits. Locally assigned addresses do not necessarily identify a manufacturer.'};
 if(id.startsWith('cidr-')||['partial-octet','host-binary','mask','broadcast','octet','host24'].includes(id)||id.startsWith('subnet-'))return {id:'notes-4-2',title:'IPv4 bits & subnet calculations',rule:'IPv4 has 32 bits. The slash number counts network bits. Host bits = 32 − prefix; total addresses = 2 raised to that host-bit count. Ordinary usable host addresses = total − 2.',example:'For /24: 32 − 24 = 8 host bits, 2⁸ = 256 total addresses, 254 usable. A mask has network 1s and host 0s. A network address has host 0s; broadcast has host 1s. /31 and /32 have special rules.'};
 return null;
}
